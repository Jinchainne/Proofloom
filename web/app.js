...
function displayValue(value) { return value === undefined || value === null ? '0' : String(value); }
async function refreshLiveStats() { const total = document.querySelector('#liveTotal'); const approved = document.querySelector('#liveApproved'); try { const data = await window.proofloomGetStats(); total.textContent = displayValue(data?.total ?? data?.[0]); approved.textContent = displayValue(data?.approved ?? data?.[1]); statsBar.classList.add('loaded'); } catch { total.textContent = 'Connect'; approved.textContent = 'wallet'; } }
