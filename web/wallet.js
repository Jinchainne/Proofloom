...
async function genlayerClient() { if (!CONTRACT_ADDRESS) throw new Error('Configure PROOFLOOM_CONTRACT_ADDRESS after deploying the contract.'); const sdk = await import('https://esm.sh/genlayer-js@2.0.0-rc.1'); const account = await connectProofloomWallet(); const client = sdk.createClient({ chain: RPC_CHAIN, account, provider: window.ethereum }); await client.connect('testnetBradbury'); return { client, account, isSuccessful: sdk.isSuccessful }; }
