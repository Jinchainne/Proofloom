...
async function genlayerClient() { if (!CONTRACT_ADDRESS) throw new Error('Configure PROOFLOOM_CONTRACT_ADDRESS after deploying the contract.'); const sdk = await import('https://esm.sh/genlayer-js@2.0.0-rc.1'); const account = await connectProofloomWallet(); const chain = sdk.testnetBradbury || RPC_CHAIN; const client = sdk.createClient({ chain, account, provider: window.ethereum }); await client.connect('testnetBradbury'); return { client, account, isSuccessful: sdk.isSuccessful }; }
...
async function submitWrite(client, write) {
  // Bradbury deployments without a FeeManager use the chain's fee-less path.
  // Fee estimation is attempted only when the chain exposes the fee manager.
  try {
    if (typeof client.estimateTransactionFeesForWrite === 'function' && client.chain?.contracts?.feeManagerContract) {
      const estimate = await client.estimateTransactionFeesForWrite(write);
      return client.writeContract({ ...write, fees: { distribution: estimate.distribution, feeValue: estimate.feeValue } });
    }
  } catch (error) {
